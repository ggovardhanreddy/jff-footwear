...
export default function ShippingCalculator({ className }: ShippingCalculatorProps) {
  ...
  const estimate = useDeliveryEstimate(state || "Andhra Pradesh");

  const check = () => {
    if (pincode.length === 6) setState(""); // triggers re-check via hooks
  };

  return (
    <div
      className={cn(
        "rounded-[24px] border border-black/5 bg-white/60 p-5 dark:border-white/10 dark:bg-brand-dark",
        className
      )}
    >
      ...
      <div className="mt-4 flex gap-2">
        ...
        <button
          type="button"
          onClick={check}
          className="shrink-0 rounded-2xl bg-brand-black px-5 py-2 text-xs font-bold uppercase tracking-wider text-white dark:bg-white dark:text-brand-black"
        >
          Check
        </button>
      </div>
      ...
      {pincode.length === 6 && delivery.checked && (
        <div className="mt-4 space-y-2 text-sm">
          <p className="flex items-center gap-2">
            ...
            {delivery.available ? "Delivery available" : SHIPPING_CONFIG.messages.deliveryUnavailable}
          </p>
          {estimate.deliveryBy && delivery.available && (
            <p className="text-emerald-600 font-medium">
              Estimated: {estimate.deliveryBy}
            </p>
          )}
          {cod.checked && (
            <p className="flex items-center gap-2">
