          ...
          {categoriesWithCount.map((category) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6">
                ...
                <p className="mt-2 text-sm text-gray-400 opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
          {MATERIALS.map((material) => (
            <Link
              ...
              className="group flex items-center justify-between border border-gray-200 bg-white p-6 transition-all hover:border-brand-accent hover:shadow-lg"
            >
              <span className="font-display font-semibold">{material}</span>
              <ArrowRight className="h-4 w-4 text-brand-muted transition-transform group-hover:translate-x-1 group-hover:text-brand-accent" />
            </Link>
          ))}
        </div>
      </div>
