...
import ProductFilterSidebar, { MobileFilterToggle } from "@/components/products/ProductFilters";
...
import { DEFAULT_FILTERS, SORT_OPTIONS, ROUTES } from "@/lib/constants";
...
export default function ProductsPageClient({ products, filterOptions }: ProductsPageClientProps) {
  ...
  if (appliedKey !== paramsKey) {
    setAppliedKey(paramsKey);
    setFilters((prev) => ({
      ...prev,
      category: (searchParams.get("category") as ProductFilters["category"]) || "",
  ...
  const departmentTitle = filters.gender
    ? filters.gender
    : filters.newArrival
      ? "New Arrivals"
      : filters.category
  ...
  const departmentDescription =
    filters.gender === "Kids"
      ? "Kids styles from the JFF catalogue. If this drop is empty, the current range is men, women, and unisex."
      : filters.gender === "Men" || filters.gender === "Women"
        ? `${filters.gender}'s styles plus unisex pairs made for everyday wear.`
        : "Browse our complete range of premium slippers. Filter by category, material, color, size, and more.";
  ...
  return (
    <div className="relative z-10 container-custom section-padding">
      ...
      <p className="mb-6 text-sm text-brand-muted">
        Showing{" "}
        <span className="font-semibold text-brand-black dark:text-white">{filtered.length}</span>{" "}
        products
      </p>

      <div className="flex flex-col gap-8 lg:flex-row">
        <aside
          className={cn("w-full shrink-0 lg:block lg:w-64", mobileFiltersOpen ? "block" : "hidden")}
        >
      ...
      <div className="mt-24 space-y-24">
        <ProductCarousel
          ...
          viewAllHref={ROUTES.collection("featured")}
        />
        <ProductCarousel
          title="New Arrivals"
          subtitle="Collection"
