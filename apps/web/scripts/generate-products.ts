...
const SHARED_OUTPUT_FILE = path.join(
  process.cwd(),
...
`;
  ...
  const sharedContent = `// Auto-generated by apps/web/scripts/generate-products.ts — do not edit manually.
...
${body}`;
  ...
  const webReexport = `// Auto-generated re-export — source of truth: @jff/shared/products
export * from "@jff/shared/products";
export { default } from "@jff/shared/products";
`;

  fs.mkdirSync(path.dirname(SHARED_OUTPUT_FILE), { recursive: true });
  fs.writeFileSync(SHARED_OUTPUT_FILE, sharedContent, "utf-8");
  fs.mkdirSync(path.dirname(OUTPUT_FILE), { recursive: true });
  fs.writeFileSync(OUTPUT_FILE, webReexport, "utf-8");
}
...
function main(): void {
  ...
  console.log(`✓ Generated ${products.length} products → packages/shared/src/products.ts`);
}
