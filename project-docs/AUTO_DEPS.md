...
| **Deploy jobs**                           | Every deploy     | Uses Node from `.nvmrc` (22 LTS line), latest npm, then `npm update` so installs float to newest versions **within** existing semver ranges. |
...
- Floating **within** `^` ranges on deploy + Dependabot PRs keeps you current without surprise breakages.
